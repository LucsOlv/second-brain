---
title: CEBRASPE
type: projeto
status: active
priority: high
start_date: 2026-04-27
tags: [trabalho, ti, fullstack, azure, consultoria]
relacionado: [[MOC - Projetos]], [[MOC - Projetos]]
---

# CEBRASPE — Consultoria de TI

> Status: 🔄 Ativo | Prioridade: 🔴 Alta | Cargo: Desenvolvedor Full Stack

```yaml
type: projeto
status: active
priority: high
start_date: 2026-04-27
tags: [trabalho, ti, fullstack, azure, consultoria]
```

## 🎯 Sobre

Consultoria de TI onde atuo como **desenvolvedor full stack**. Desenvolvimento de sistemas web para a organização CEBRASPE.

## 📋 Info Profissional

| Campo | Valor |
|---|---|
| **Email** | lucas.oliveira@consultor.cebraspe.org.br |
| **Cargo** | Desenvolvedor Full Stack |
| **Ambiente Dev** | https://devsistemas.cebraspe.org.br |
| **Azure DevOps** | https://dev.azure.com/Cebraspe/ |

## ☁️ Infraestrutura

| Plataforma | URL |
|---|---|
| Azure DevOps (repositórios) | https://dev.azure.com/Cebraspe/ |
| Ambiente de Desenvolvimento | https://devsistemas.cebraspe.org.br |

## 🛠️ Stack Tecnológica

[?] — Quais tecnologias são usadas? (Node, .NET, React, etc.)

## 📋 Repositórios Azure

| Repositório | Descrição |
|---|---|
| [?] | [?] |
| [?] | [?] |

## 📊 Progresso

| Fase | Status | Nota |
|---|---|---|
| [?] Desenvolvimento | 🔄 Em progresso | Tarefa: melhoria de atendimento |
| [?] Código | 🔄 Em progresso | |
| [?] Code Review | ⬜ | |
| [?] Deploy | ⬜ | |

## 📝 Tarefas Activas

| # | Descrição | Status | Sprint |
|---|---|---|---|
| ? | Melhoria de atendimento | 🔜 | [?] |

## 📝 Notas e Logs

### Log de Trabalho

| Data | Actividade | Nota |
|---|---|---|
| 2026-04-27 | Melhoria de atendimento | Primeira tarefa registada |

---

## 🔗 Conectado a

- [[MOC - Projetos]]
- [[MOC - Tarefas]]
- [[PRION]] — projeto de pesquisa pessoal (separado)
- [[Hermes Agent]] — possível integração de lembretes